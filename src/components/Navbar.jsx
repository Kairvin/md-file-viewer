    ...
    <header id="navbar" className="h-14 sm:h-16 border-b border-slate-200 dark:border-slate-800 bg-white/95 dark:bg-slate-900/95 backdrop-blur-md sticky top-0 z-40 px-2 sm:px-4 flex items-center justify-between transition-colors">
      ...
      <div className="flex items-center gap-1 sm:gap-1.5">
        ...
        {onOpenTools && (
          <button
            onClick={onOpenTools}
            className="px-2.5 py-1.5 rounded-xl border border-slate-200 dark:border-slate-700 bg-white dark:bg-slate-800 hover:bg-slate-50 dark:hover:bg-slate-700/80 text-slate-700 dark:text-slate-200 transition-all text-xs font-semibold flex items-center gap-1.5 shadow-2xs mr-1"
            title="Document Tools Hub (.md, .docx, .pptx)"
          >
            <Wrench className="w-3.5 h-3.5 text-blue-500" />
            <span className="hidden sm:inline">Tools</span>
            <span className="text-[10px] px-1.5 py-0.2 rounded-full font-bold uppercase tracking-wider bg-blue-50 dark:bg-blue-950/60 text-blue-600 dark:text-blue-400 border border-blue-200 dark:border-blue-800">
          ...
          <div className="fixed right-0 top-0 bottom-0 w-80 max-w-[85vw] bg-white dark:bg-slate-900 p-5 shadow-2xl flex flex-col justify-between overflow-y-auto animate-in slide-in-from-right-full duration-200">
            <div>
              ...
              <div className="mt-4 space-y-1">
                <div className="text-[11px] font-bold text-slate-400 uppercase tracking-wider px-1 mb-2">Document Tools</div>
                {onOpenTools && (
                  <button
                    onClick={() => { onOpenTools(); setShowMobileMenu(false); }}
                    className="w-full text-left px-3 py-2 rounded-xl bg-blue-50/70 dark:bg-blue-950/40 hover:bg-blue-100/60 dark:hover:bg-blue-900/60 text-xs font-semibold flex items-center justify-between text-blue-600 dark:text-blue-400 border border-blue-200/80 dark:border-blue-800/80 mb-2"
                  >
                    <span className="flex items-center gap-2">
                      <Wrench className="w-4 h-4 text-blue-500" />
                      <span>Document Tools Hub</span>
                    </span>
                    <span className="text-[10px] px-1.5 py-0.2 rounded-full uppercase tracking-wider font-bold bg-blue-100 dark:bg-blue-900 text-blue-700 dark:text-blue-300">
                      {activeTool === 'word' ? 'DOCX' : activeTool === 'pptx' ? 'PPTX' : 'MD'}
                    </span>
                  </button>
                )}
                ...
                <div className="grid grid-cols-2 gap-2">
                  ...
                  <button
                    ...
                    className={`col-span-2 p-2.5 rounded-xl border text-xs font-medium flex items-center justify-between transition-all ${
                      viewMode === 'playground' 
                        ? 'border-slate-300 dark:border-slate-700 bg-slate-100 dark:bg-slate-800 text-slate-900 dark:text-white font-semibold' 
                  ...
                  >
                    ...
                    <span className="text-[10px] bg-slate-200 dark:bg-slate-700 text-slate-800 dark:text-slate-200 px-1.5 py-0.5 rounded font-semibold">New</span>
                  </button>
                </div>
              </div>
